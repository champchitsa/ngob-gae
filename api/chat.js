...
function buildEvidenceAnswer(question, selectedCases, selectedItems) {
  ...
  if (selectedItems.length) {
    const sections = selectedItems.slice(0, 3).map((item, index) => `${index + 1}. [รายการ ${index + 1}] ${item.item}\nหน่วยงาน: ${item.agency}\nวงเงินตาม พ.ร.บ. ${item.act.toLocaleString('th-TH')} ล้านบาท หลังโอน ${item.adjusted.toLocaleString('th-TH')} ล้านบาท เบิกจ่ายรวม PO ${item.committed.toLocaleString('th-TH')} ล้านบาท\nสัญญาณคัดกรอง: ${item.signals.join(', ')}`)
    return `รายการที่ค้นพบจากข้อมูล PBO 2568\n\n${sections.join('\n\n')}\n\nสัญญาณเหล่านี้ใช้จัดลำดับการตรวจเอกสารเท่านั้น ควรเชื่อม TOR ราคากลาง สัญญา การแก้ไขสัญญา และผลตรวจรับก่อนสรุป`
  }
