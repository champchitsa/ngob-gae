...
const datasets: { id: DatasetId; label: string; count: string; detail: string }[] = [
  ...
  { id: 'structure', label: 'โครงสร้างรัฐเชื่อมงบ', count: '417', detail: 'กระทรวง หน่วยงาน ภารกิจ หน่วยย่อย วงเงิน และรายการที่ผ่านเกณฑ์คัดกรอง' },
...
const number = (value?: number | null, digits = 1) => value === undefined || value === null ? 'ไม่พบข้อมูล' : new Intl.NumberFormat('th-TH', { maximumFractionDigits: digits }).format(value)
const bytes = (value?: number) => {
  if (value === undefined) return 'ไม่พบข้อมูล'
